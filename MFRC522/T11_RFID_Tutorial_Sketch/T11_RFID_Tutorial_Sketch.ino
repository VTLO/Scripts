...
byte blockcontent_id[16] = {"1234 1234 1234"};//an array with 16 bytes to be written into one of the 64 card blocks is defined
byte blockcontent_name[16] = {"Kushagra Karira"};
byte blockcontent_info[16] = {"19, O+, Normal"};
...
void loop()
{
	...
	if ( ! mfrc522.PICC_ReadCardSerial()) {//if PICC_ReadCardSerial returns 1, the "uid" struct (see MFRC522.h lines 238-45)) contains the ID of the read card.
		...
		//byte		sak;			// The SAK (Select acknowledge) byte returned from the PICC after successful selection.
         ...
         writeBlock(1, blockcontent_id);//the blockcontent array is written into the card block
         writeBlock(2, blockcontent_name);
         writeBlock(4, blockcontent_info);
